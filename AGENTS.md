...
1. **Launch as an executable, not a project.** Aspire cannot add a project it can't reference at compile time, so the external project is added via `builder.AddExecutable(name, "dotnet", folder, ["run", "--project", <csproj>, "--no-launch-profile", ...])`. `dotnet run` handles build+run. When `UseWatch` is set, the command is `dotnet watch --non-interactive --project <csproj> run ...` instead, for recompile/hot-reload on source changes.
...
3. **`WithExecutableProjectDefaults`** is a hand-ported copy of Aspire's internal `WithProjectDefaults`: it sets the OTLP/OpenTelemetry env vars, calls `WithOtlpExporter()`, reads the external project's `appsettings.json` / `appsettings.{env}.json` to discover Kestrel endpoints, materializes endpoints from the launch profile's `applicationUrl`, builds `ASPNETCORE_URLS`, and (via `WithUrls`) combines the profile's `launchUrl` with each endpoint URL so the dashboard shows the launch/OpenAPI URL like `AddProject` does. **This mirrors Aspire internals** — when bumping the Aspire dependency, re-check this method against the upstream `WithProjectDefaults`.
