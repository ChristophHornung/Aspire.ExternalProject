...
string path = Path.Combine(Directory.GetCurrentDirectory(), "..", "External");
string weatherApiPath = Path.Combine(path, "WeatherApi", "WeatherApi.csproj");
string predictionApiPath = Path.Combine(path, "Weather.Prediction.Api", "Weather.Prediction.Api.csproj");
...
builder.AddExternalProject("WeatherApi", weatherApiPath, cfg =>
{
...
builder.AddExternalProject("WeatherPredictionApi", predictionApiPath, cfg =>
{
	// Skipping git support will not wire the 'Git pull' command.
	cfg.SkipGitSupport = false;
